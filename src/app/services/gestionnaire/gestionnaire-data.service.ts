...
export class GestionnaireDataService {
  ...
  demandeContributeurParProjet(id: number) {
    return this.data.getData(
      Env.PROJET + '/' + id + '/demandes-contribution/v2'
    );
  }

  //Accepter une demande de contribution :
  demandeAccepter(id: number, est: boolean) {
    return this.data.patchData(
      Env.PROJET + '/demandes-contribution/' + id + '/estAcceptee/' + est,
      null
    );
  }

  //Trouver un Gestionnaire par son iD Utilisateur
  trouverUnGestionnaireParsonidutilisateur(
    id: number
  ): Observable<GestionnaireDto> {
    return this.data.getData(`${Env.GET_GESTIONNAIRE}/${id}/gestionnaire`);
  }
}
